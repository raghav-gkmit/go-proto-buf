...
type HelloServiceClient interface {
	...
	SayHelloContinuos(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[HelloRequest, HelloResponse], error)
}
...
func (c *helloServiceClient) SayHelloContinuos(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[HelloRequest, HelloResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &HelloService_ServiceDesc.Streams[2], HelloService_SayHelloContinuos_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[HelloRequest, HelloResponse]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type HelloService_SayHelloContinuosClient = grpc.BidiStreamingClient[HelloRequest, HelloResponse]
...
type HelloServiceServer interface {
	...
	SayHelloContinuos(grpc.BidiStreamingServer[HelloRequest, HelloResponse]) error
...
func (UnimplementedHelloServiceServer) SayHelloContinuos(grpc.BidiStreamingServer[HelloRequest, HelloResponse]) error {
...
func _HelloService_SayHelloContinuos_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(HelloServiceServer).SayHelloContinuos(&grpc.GenericServerStream[HelloRequest, HelloResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type HelloService_SayHelloContinuosServer = grpc.BidiStreamingServer[HelloRequest, HelloResponse]
